...
import { Flag, Search, Filter, Loader2, Sparkles, TrendingUp, CheckCircle2, Activity, Clock } from 'lucide-react'
...
export default function Milestones() {
  ...
  const filteredMilestones = (milestones || []).filter(ms => {
    const matchesSearch = ms.title.toLowerCase().includes(search.toLowerCase()) || 
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-slate-500 font-medium">Loading your recovery path...</p>
      </div>
    )
  }
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div>
            <div className="flex items-center gap-2 text-[var(--primaryGreen)] mb-2">
              <Flag size={20} />
              <span className="text-sm font-bold uppercase tracking-widest">Milestones</span>
            </div>
            <h1 className="serif-heading text-4xl text-slate-900 mb-4">Daily Recovery Track</h1>
            <p className="text-slate-500 max-w-xl text-lg leading-relaxed">
              Complete your daily rehabilitation tasks to progress through your treatment. Each step is medically guided for optimal recovery.
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4 bg-slate-50 p-6 rounded-[32px]">
            <div className="flex items-center gap-4 px-4">
               <div className="text-center">
                  <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Progress</div>
          ...
          {['all', 'available', 'completed', 'locked'].map(f => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all ${
                filter === f ? 'bg-[var(--primaryGreen)] text-white shadow-lg' : 'text-slate-500 hover:text-slate-900'
              }`}
            >
      ...
      <div className="max-w-5xl mx-auto py-10">
        {filteredMilestones.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-[40px] border border-dashed border-slate-200">
            ...
            <h3 className="text-xl font-bold text-slate-800">No milestones found</h3>
      ...
      <AnimatePresence>
        {showDetails && selectedMilestone && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setShowDetails(false)} className="absolute inset-0 bg-slate-900/40 backdrop-blur-md" />
            <motion.div 
            ...
            >
              <div className="p-10">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="serif-heading text-3xl text-slate-900 leading-tight">{selectedMilestone.title}</h3>
                    <div className="flex items-center gap-4 text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
                      <Clock size={14} />
                      {selectedMilestone.duration} mins • {selectedMilestone.difficulty}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div>
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-3 flex items-center gap-2">
                      <Activity size={14} className="text-[var(--primaryGreen)]" />
                      About this milestone
                    </h4>
                    <p className="text-slate-600 leading-relaxed">{selectedMilestone.description}</p>
                  </div>

                  <div className="p-8 bg-slate-50 rounded-[32px] border border-slate-100">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-4">Step-by-step instructions</h4>
                    <div className="text-slate-700 space-y-4">
                      {selectedMilestone.instructions.split('\n').map((line, i) => (
