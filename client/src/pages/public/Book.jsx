...
export default function Book() {
  ...
  const doctorCards = realDoctors.length ? realDoctors : [
    ...
    { id: 4, name: 'Dr. Michael Barnes', specialization: 'Chronic Pain Management', experience: '10 Years', rating: '4.7', bio: 'Focused on holistic approaches to persistent pain conditions and improving daily function.' }
  ]
    ...
    <main className="min-h-screen bg-[var(--mutedWhite)] font-sans relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-32 pb-24 relative z-10">
        <div className="mb-16 max-w-2xl">
          ...
          >
            Start your <span className="italic font-light text-[var(--primaryGreen)]">recovery.</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Select a world-class specialist and schedule your initial consultation. We'll handle the rest.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-20 items-start">
          ...
          <div className="space-y-8">
            <h2 className="text-3xl font-serif text-[var(--textDark)]">Select a Specialist</h2>
                ...
                return (
                  ...
                  >
                    {isSelected && (
                      ...
                      >
                        <CheckCircle2 size={24} className="fill-[var(--primaryGreen)] text-white" />
                      </motion.div>
                    )}
                    ...
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <div className="pr-8">
                        <h3 className="font-bold text-xl text-[var(--textDark)] line-clamp-1">{doctor.name}</h3>
                        <p className="text-sm font-medium text-[var(--primaryGreen)] mb-2">{doctor.specialization || 'Rehab Specialist'}</p>
                        <div className="flex items-center gap-1 text-xs font-semibold text-[var(--textDark)] bg-white/60 backdrop-blur-sm w-fit px-2 py-0.5 rounded-full">
                          <Star size={12} className="fill-yellow-500 text-yellow-500" /> {doctor.rating || '5.0'}
                        </div>
                      </div>
                    </div>
                    
                    <p className="text-sm text-[var(--textSoft)] leading-relaxed mb-6 line-clamp-3">
                      {doctor.bio || 'Focused on restoring mobility with compassionate care and evidence-based treatments.'}
                    </p>
                    ...
                    <div className="pt-5 border-t border-[var(--borderSoft)] flex items-center justify-between">
                      <div className="text-xs text-[var(--textSoft)]">
                        <span className="block font-semibold text-[var(--textDark)]">{doctor.experience || '10+ Years'}</span>
                        Experience
                      </div>
                    </div>
                  </motion.div>
              ...
              {bookingMutation.isSuccess ? (
                ...
                >
                  <div className="w-24 h-24 rounded-full bg-[var(--softLime)] flex items-center justify-center mx-auto mb-8">
                    <CheckCircle2 size={48} className="text-[var(--darkGreen)]" />
                  </div>
                  <h3 className="text-4xl font-serif text-[var(--textDark)] mb-4">Request Sent!</h3>
                  <p className="text-[#5C6B63] text-lg mb-8">
                    Your appointment request has been securely forwarded. We will email you with patient access credentials once approved.
                  </p>
                  <Button onClick={() => bookingMutation.reset()}>Book Another Session</Button>
                </motion.div>
              ) : (
                <motion.div
                  initial={{ opacity: 0 }}
                  ...
                  exit={{ opacity: 0 }}
                >
                  <h3 className="text-3xl font-serif text-[var(--textDark)] mb-2">Request Session</h3>
                  <p className="text-base text-[#5C6B63] mb-10 pb-6 border-b border-[var(--borderSoft)]">
                    {selectedDoctor ? (
                      <span className="flex items-center gap-2">
                        Booking with <strong className="text-[var(--textDark)]">{selectedDoctor.name}</strong>
                      </span>
                    ) : (
                  ...
                  <form className="space-y-10" onSubmit={submit}>
                    ...
                    <div className="space-y-6">
                      <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--textSoft)]">Patient Information</h4>
                    ...
                    {/* Date & Time Selection */}
                    <div className={`space-y-8 transition-opacity duration-300 ${!selectedDoctor ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
                      
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--textSoft)]">Select Date</h4>
                          {slotsQuery.isLoading && <span className="text-xs text-[var(--primaryGreen)] animate-pulse">Loading availability...</span>}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          {!slotsQuery.isLoading && availableDates.length === 0 && selectedDoctor && (
                            <div className="text-sm text-red-500 bg-red-50 px-4 py-3 rounded-2xl border border-red-100">No availability this week.</div>
                          )}
                          {!selectedDoctor && (
                            <div className="text-sm text-[var(--textSoft)] bg-white/50 px-4 py-3 rounded-2xl border border-dashed border-[#DCE3DA] w-full">Pending doctor selection...</div>
                          )}
                        </div>
                      </div>

                      <div className="space-y-4">
                        <h4 className="text-sm font-bold uppercase tracking-wider text-[var(--textSoft)]">Select Time</h4>
                        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-3 gap-3">
                          ...
                          {selectedDate && visibleSlots.length === 0 && (
                            <div className="col-span-full text-sm text-[var(--textSoft)]">No slots available for this date.</div>
                          )}
                          {!selectedDate && selectedDoctor && availableDates.length > 0 && (
                            <div className="col-span-full text-sm text-[var(--textSoft)] bg-white/50 px-4 py-3 rounded-2xl border border-dashed border-[#DCE3DA]">Please select a date first.</div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="pt-4 border-t border-[var(--borderSoft)]">
                      <Button 
                        type="submit" 
                        disabled={bookingMutation.isPending || !selectedDoctor || !slotAt || !name || !email} 
                        className="group w-full"
                      >
                        {bookingMutation.isPending ? 'Submitting Request...' : 'Confirm Appointment'}
                        {!bookingMutation.isPending && <ArrowRight size={20} className="ml-2 group-hover:translate-x-1 transition-transform" />}
                      </Button>
                    </div>
                  </form>
                </motion.div>
